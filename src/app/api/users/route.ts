...
export async function GET() {
  ...
  const rows = await db.query.users.findMany({
    columns: { id: true, name: true, email: true, department: true, role: true, createdAt: true },
...
export async function POST(request: Request) {
  ...
  if (!name || !email || !password) {
    return NextResponse.json({ error: "姓名、電子郵件、密碼皆為必填" }, { status: 400 });
  }
  ...
  const [created] = await db
    .insert(users)
    .values({ name, email, department, passwordHash, role: "member" })
