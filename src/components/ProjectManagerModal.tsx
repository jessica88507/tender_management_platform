...
export function ProjectManagerModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="bg-card border border-border rounded-[10px] py-5 px-5.5 shadow-[0_8px_24px_rgba(0,0,0,0.25)] max-w-[760px] w-[90vw]">
        <h2 className="font-serif text-[26px] mb-4">專案管理</h2>
        {ids.length === 0 ? (
          <div className="font-serif text-[23.5px] text-ink-soft mt-5">目前沒有任何案件。</div>
        ) : (
                  ...
                  <tr key={id}>
                    ...
                    <td className="border-b border-dashed border-border py-2 px-2">
                      <button
                        ...
                        className="bg-transparent border border-accent text-accent rounded-md py-1 px-2.5 text-[15px] cursor-pointer hover:bg-accent/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
                      >
                        開啟
                      </button>
                    </td>
